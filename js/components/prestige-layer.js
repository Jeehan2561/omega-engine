...
<p v-if="layer.hasSimpleBoost()" class="power-text">This translates to a <span class="big-number">x{{format(layer.getSimpleBoost(), 3)}}</span> Boost on Layer 1 Production</p>
<div class="resource-button" v-if="layer.hasResourceButton"><button @click="layer.addResource(layer.getResourceButtonAmount())">+{{format(layer.getResourceButtonAmount(), 3)}} <resource-name :layerid="layer.layer"></resource-name></button></div>
<button v-if="nextLayer && (layer.canGenerateNextLayer() || nextLayer.timesReset > 0)" :disabled="!layer.canPrestige()" class="prestige" @click="layer.prestige()">
    <span v-if="layer.isNonVolatile()">+{{format(layer.getPrestigeAmountPerSecond(), 3)}} <resource-name :layerid="nextLayer.layer"></resource-name>/s</span>
    <span v-else>
        <span v-if="layer.canPrestige()">Prestige to go <resource-name :layerid="nextLayer.layer"></resource-name><br/>Get +{{format(layer.getPrestigeAmount(), 3)}} <resource-name :layerid="nextLayer.layer"></resource-name></span>
        <span v-else>Reach {{format(layer.getPrestigeLimit(), 3)}} <resource-name :layerid="layer.layer"></resource-name></span>
    </span>
</button>
<div class="tabs">
...
<div v-if="layer.hasPower() && tab === TAB_POWER">
    <p class="power-text">You have <span class="big-number">{{format(layer.power, 3)}}</span> <resource-name :layerid="layer.layer"></resource-name>-Power, 
    translated to a <span class="big-number">x{{format(layer.getPowerBoost(), 3)}}</span> Boost on Layer 1 Generators</p>
